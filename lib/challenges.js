...
export function validateRun(run) {
  ...
  const totalDistance = TRACK.laps * getTrackLength();
  ...
  if (Number(run.coins) > PICKUPS.length) return "more coins than exist on the course";
  if (Number(run.boostUses) > 3) return "more boosts than a run allows";
  return null;
}
