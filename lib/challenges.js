...
import { COINS_PER_BOOST, MAX_BOOST_CHARGES } from "../game/vehicle.js";
...
const TRACKS = Object.fromEntries(
  listTracks().map((t) => [t.id, {
    laps: t.laps,
    totalDistance: t.laps * t.totalLength,
    pickupCount: t.pickupCount,
    boostStarCount: t.boostStarCount || 0,
  }]),
);
...
// The recorded cumulative distance has occasional discontinuities that are not
// real speed: the per-lap re-bin (distance wraps a beat before the lap counter
// catches up), and projection snaps when a frame hitch jumps the car's position
// — common on phones, and worst on the self-crossing city circuit. Each spikes
// a few isolated segments. A genuine time-compression forgery instead pushes a
// large FRACTION of the whole trace over the ceiling, so we judge by proportion
// rather than an absolute count: a legit run with a handful of glitches is never
// rejected, while a forged run (>10% of segments over-speed) still is.
const MAX_SPEED_VIOLATION_RATIO = 0.1;
const MIN_SPEED_VIOLATIONS = 8;
...
export function validateRun(run) {
  ...
  for (const sample of ghost) {
    ...
    if (prev) {
      ...
      segments += 1;
      const dd = sample.d - prev.d;
      // backward / zero distance is a lap re-bin dip or reversing, never a
      // forward-speed claim — only forward over-speed segments count.
      if (dd > 0 && dd / ((sample.t - prev.t) / 1000) > MAX_PLAUSIBLE_SPEED) speedViolations += 1;
    }
    maxDistance = Math.max(maxDistance, sample.d);
    prev = sample;
  }
  // A few over-speed segments are recording glitches; a forgery over-speeds a
  // large share of the whole trace (see MAX_SPEED_VIOLATION_RATIO).
  if (speedViolations > MIN_SPEED_VIOLATIONS && speedViolations > segments * MAX_SPEED_VIOLATION_RATIO) {
    return "impossible speed in ghost trace";
  }
  ...
  // Boost stars also respawn per lap and each can refill up to MAX_BOOST_CHARGES,
  // so they raise the ceiling on how many boosts a clean run could fire.
  const coins = Number(run.coins) || 0;
  if (coins > track.pickupCount * track.laps) return "more coins than exist on the course";
  const maxBoostUses = 3 + Math.floor(coins / COINS_PER_BOOST) + track.boostStarCount * track.laps * MAX_BOOST_CHARGES;
  if (Number(run.boostUses) > maxBoostUses) return "more boosts than the run could earn";
  return null;
}
