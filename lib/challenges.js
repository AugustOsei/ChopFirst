...
// At every lap boundary the ghost's cumulative distance re-bins for a single
// tick — the spline distance wraps to ~0 a beat before the lap counter catches
// up — producing one isolated teleport spike of roughly a whole lap length
// (thousands of m/s). That is an artifact of how the trace is recorded, not a
// claimable speed: the physical-floor check below caps how far a time can be
// lowered, so a genuinely forged-fast run shows over-speed only modestly above
// the ceiling and sustained across many ticks. We count those; we ignore the
// lap re-bin spikes above this ceiling.
const REBIN_ARTIFACT_SPEED = MAX_PLAUSIBLE_SPEED * 5;
...
export function validateRun(run) {
  ...
  for (const sample of ghost) {
    ...
    if (prev) {
      ...
      // Count over-speed only within the plausibly-forgeable band; the giant
      // per-lap re-bin spikes (see REBIN_ARTIFACT_SPEED) are recording
      // artifacts, not claimable speeds, so they never count against the run.
      if (speed > MAX_PLAUSIBLE_SPEED && speed < REBIN_ARTIFACT_SPEED) speedViolations += 1;
    }
