...
// Player feedback (bug reports, feature ideas, Ananse pace reports). Newest
// first, capped so a spammer can't bloat the store. Never pruned with challenges.
...
export async function addFeedback(payload) {
  ...
  store.feedback.unshift({
    ...
    type: ["idea", "pace"].includes(payload.type) ? payload.type : "bug",
    message,
    // race context for "pace" reports: difficulty, finishing time, margin
    context: String(payload.context || "").slice(0, 160),
